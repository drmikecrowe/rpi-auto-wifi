...
class WifiUtil:
    ...
    @classmethod
    def wps_connect(cls, bssid):
        return WpaHelper.command(WPA_CMD.WPS_CONNECT, bssid)

    @classmethod
    def reconnect(cls):
        return WpaHelper.command(WPA_CMD.RECONNECT)

    @classmethod
    def add_network(cls, ssid, password=None):
        network_id = None
    ...
    def network_repair(cls):
        ...
        if not ok:
            msg.append(
                "FOUND: wireless interface status:{}".format(status.get("wpa_stat"))
            )
        try:
